...
export default function PricingUpdateForm(props) {
  const {
    id: idProp,
    pricing: pricingModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = pricingRecord
      ? { ...initialValues, ...pricingRecord }
      : initialValues;
    setPlan(cleanValues.plan);
    setPrice(cleanValues.price);
    setDetails(cleanValues.details ?? []);
    setCurrentDetailsValue("");
    setCatchPhase(cleanValues.catchPhase);
    setErrors({});
  };
  const [pricingRecord, setPricingRecord] = React.useState(pricingModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Pricing, idProp)
        : pricingModelProp;
      setPricingRecord(record);
    };
    queryData();
  }, [idProp, pricingModelProp]);
  React.useEffect(resetStateValues, [pricingRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Pricing.copyOf(pricingRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "PricingUpdateForm")}
      {...rest}
    >
      <TextField
        label="Plan"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || pricingModelProp)}
