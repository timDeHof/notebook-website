...
export default function PageUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = pageRecord
      ? { ...initialValues, ...pageRecord, Subject, Notebook }
      : initialValues;
    ...
    setNotebook(cleanValues.Notebook);
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Page, idProp)
        : pageModelProp;
      setPageRecord(record);
      const SubjectRecord = record ? await record.Subject : undefined;
      setSubject(SubjectRecord);
      const NotebookRecord = record ? await record.Notebook : undefined;
      setNotebook(NotebookRecord);
    };
    queryData();
  }, [idProp, pageModelProp]);
  React.useEffect(resetStateValues, [pageRecord, Subject, Notebook]);
        ...
        try {
          ...
          await DataStore.save(
            Page.copyOf(pageRecord, (updated) => {
              ...
              if (!modelFields.Subject) {
                updated.subjectID = undefined;
              }
              if (!modelFields.Notebook) {
                updated.notebookID = undefined;
              }
            })
          );
          if (onSuccess) {
