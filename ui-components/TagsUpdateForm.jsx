...
export default function TagsUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = tagsRecord
      ? { ...initialValues, ...tagsRecord, notes: linkedNotes }
      : initialValues;
    setTagName(cleanValues.tagName);
    setTagDesc(cleanValues.tagDesc);
  ...
  React.useEffect(() => {
    const queryData = async () => {
      ...
      const linkedNotes = record
        ? await Promise.all(
            (
              await record.notes.toArray()
            ).map((r) => {
              return r.notes;
            })
          )
  ...
  React.useEffect(resetStateValues, [tagsRecord, linkedNotes]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const promises = [];
          const notesToLinkMap = new Map();
          const notesToUnLinkMap = new Map();
          const notesMap = new Map();
          const linkedNotesMap = new Map();
          notes.forEach((r) => {
            const count = notesMap.get(getIDValue.notes?.(r));
            const newCount = count ? count + 1 : 1;
            notesMap.set(getIDValue.notes?.(r), newCount);
          });
          linkedNotes.forEach((r) => {
            const count = linkedNotesMap.get(getIDValue.notes?.(r));
            const newCount = count ? count + 1 : 1;
            linkedNotesMap.set(getIDValue.notes?.(r), newCount);
          });
          linkedNotesMap.forEach((count, id) => {
            const newCount = notesMap.get(id);
            if (newCount) {
              const diffCount = count - newCount;
              if (diffCount > 0) {
                notesToUnLinkMap.set(id, diffCount);
              }
            } else {
              notesToUnLinkMap.set(id, count);
            }
          });
          notesMap.forEach((count, id) => {
            const originalCount = linkedNotesMap.get(id);
            if (originalCount) {
              const diffCount = count - originalCount;
              if (diffCount > 0) {
                notesToLinkMap.set(id, diffCount);
              }
            } else {
              notesToLinkMap.set(id, count);
            }
          });
          notesToUnLinkMap.forEach(async (count, id) => {
            const tagsNotesRecords = await DataStore.query(TagsNotes, (r) =>
              r.and((r) => {
                const recordKeys = JSON.parse(id);
                return [
                  r.notesId.eq(recordKeys.id),
          ...
          notesToLinkMap.forEach((count, id) => {
            for (let i = count; i > 0; i--) {
              promises.push(
                DataStore.save(
                  new TagsNotes({
                    tags: tagsRecord,
                    notes: notesRecords.find((r) =>
