...
export default function NotebookUpdateForm(props) {
  const {
    id: idProp,
    notebook: notebookModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = notebookRecord
      ? { ...initialValues, ...notebookRecord, Subject, Pages: linkedPages }
      : initialValues;
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Notebook, idProp)
        : notebookModelProp;
      setNotebookRecord(record);
      const SubjectRecord = record ? await record.Subject : undefined;
      setSubject(SubjectRecord);
      const linkedPages = record ? await record.Pages.toArray() : [];
      setLinkedPages(linkedPages);
    };
    queryData();
  }, [idProp, notebookModelProp]);
  React.useEffect(resetStateValues, [notebookRecord, Subject, linkedPages]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          pagesToUnLink.forEach((original) => {
            if (!canUnlinkPages) {
              throw Error(
                `Page ${original.id} cannot be unlinked from Notebook because undefined is a required field.`
              );
            }
            promises.push(
              DataStore.save(
          ...
          pagesToLink.forEach((original) => {
            promises.push(
              DataStore.save(
                Page.copyOf(original, (updated) => {
                  updated.Notebook = notebookRecord;
                })
              )
            );
          });
          ...
          promises.push(
            DataStore.save(
              Notebook.copyOf(notebookRecord, (updated) => {
                Object.assign(updated, modelFieldsToSave);
                if (!modelFieldsToSave.Subject) {
                  updated.notebookSubjectId = undefined;
                }
              })
            )
          );
      ...
      {...getOverrideProps(overrides, "NotebookUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || notebookModelProp)}
